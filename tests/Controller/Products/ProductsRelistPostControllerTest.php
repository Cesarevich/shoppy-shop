...
final class ProductsRelistPostControllerTest extends WebTestCase
{
    #[Test]
    public function it_should_relist_a_product(): void
    {
        ...
        $client->jsonRequest('POST', '/products/' . $productId . '/relist');
        self::assertResponseStatusCodeSame(204);
        ...
        self::assertSame('onSale', $data['listingStatus']);
    }

    #[Test]
    public function it_should_reject_product_relist_if_already_on_sale(): void
    {
        $productId = ProductId::random()->value();
        $typeId = TypeId::random()->value();
        $client = self::createClient();
        ...
        $data = json_decode((string) $client->getResponse()->getContent(), true, 512, JSON_THROW_ON_ERROR);
        self::assertSame('product_already_on_sale', $data['error']);
    }

    #[Test]
    public function it_should_reject_product_relist_if_not_withdrawn(): void
    {
        $productId = ProductId::random()->value();
        $typeId = TypeId::random()->value();
        $client = self::createClient();
        ...
        $data = json_decode((string) $client->getResponse()->getContent(), true, 512, JSON_THROW_ON_ERROR);
        self::assertSame('product_not_withdrawn', $data['error']);
    }
